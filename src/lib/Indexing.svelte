  ...
  let complete_rescan = false;   // bind to checkbox.
    async function startIndex(){
        ...
        if (index_start_button && index_directory_path)
        {
            ...
            let url = "/indexStart"
            let response = await fetch(DOMAIN + url, {method: 'POST', body: form_data}) ;
            let data = await response.json();
...
<div class="container bg-white  max-w-lg my-8 p-4 shadow p-4">
    <p class="mb-2 text-sm font-semibold">Press Start button to start indexing</p>
  
    <div class="my-2">
      <input bind:this={input_element} bind:value={index_directory_path} on:keydown={(e) => {if(e.key === "Enter"){index_start_button.click();}}} type="text" class="w-full focus:outline-none rounded border py-2 px-4 text-sm" placeholder="D://images (Full path to directory to index)" required={true}/>
    </div>
