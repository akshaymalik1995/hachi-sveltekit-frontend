<script>
    ...
    function clearFilter(target, option, value) {
        selectedFilters[option] = selectedFilters[option].filter(
            (item) => item !== value
        );
    ...
    async function handleFormSubmit(e) {
        ...
        for (let key of temp_keys) {
            if (selectedFilters[key].length > 0) {
                ...
                finalQuery += key + "=" + selectedFilters[key].map(encodeURIComponent).join("?");
            }
        }
    ...
    const placeholders = {
        person: "Akshay, Neha, etc",
        query: "dogs, cats, etc",
        place: "Mumbai, Delhi, etc",
        filename: "IMG_1234, IMG_5678, etc",
    };
</script>
...
<div class="w-full py-4 mx-auto">
    <div class="w-full relative place-content-center">
                        ...
                        >
                            ...
                            >
                                {#each selectedFilters[option] as value}
                                    <span class="ml-1 p-2 rounded-md">{value}</span>
                                    <div
                                        class="ml-2 text-red-600 cursor-pointer hover:text-blue-500"
                                        on:click={(e) => {
                                            e.stopPropagation();
                                            clearFilter(e.currentTarget, option, value);
                                        }}
                                    >
        ...
        {#if dropdownItems.length > 0}
            ...
            >
                <ul
                    class="divide-y bg-gray-800 text-white max-h-36 overflow-y-auto divide-gray-200"
                >
                    {#each dropdownItems as item}
                        <!-- svelte-ignore a11y-click-events-have-key-events -->
                        <!-- svelte-ignore a11y-no-noninteractive-element-interactions -->
                        <li
                            on:click={() => handleListItemClick(selectedOption, item)}
